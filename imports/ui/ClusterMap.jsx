...
import { Map, Marker, Polyline, TileLayer, CircleMarker, Popup, LayerGroup } from 'react-leaflet';
...
export default class ClusterMap extends Component {
  ...
  render() {
    ...
    var coordinates = this.props.cluster.locations.coordinates.filter(function(l) {
    ...
    var latitudes = coordinates.map(function(l) { return l[1]; })
    var longitudes = coordinates.map(function(l) { return l[0]; })

    var center_latitude = latitudes.sum()/coordinates.length;
    var center_longitude = longitudes.sum()/coordinates.length;

    var reversed_coords = coordinates.map(function(l) { return [l[1], l[0]]; });
    ...
    for (var i = 1; i < coordinates.length; i++) {
      var corrected_start_time = moment(this.props.cluster.times[i].utc_timestamp).utcOffset(this.props.cluster.times[i].tz_offset/60);
      var corrected_end_time = moment(this.props.cluster.times[i-1].utc_timestamp).utcOffset(this.props.cluster.times[i-1].tz_offset/60);
    ...
    if ('bounds' in this.props) {
      ...
      // for maps on the cluster calendar page, we want to have them offset to the right a significant bit
    ...
    if ('callback' in this.props) {
      return (
          <Map key={this.props.cluster._id + "_map"} bounds={bounds} ref={(map) => {this.map = map}} onMoveEnd={this.props.callback} animate={false}>
            <TileLayer
      ...
      return (
          <Map key={this.props.cluster._id + "_map"} bounds={bounds} ref={(map) => {this.map = map}} animate={false}>
            <TileLayer
              key = {this.props.cluster._id._str + "_tilelayer"}
              url = 'http://{s}.basemaps.cartocdn.com/light_all/{z}/{x}/{y}.png'
              subdomains='abcd'
